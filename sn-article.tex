...
\label{sec:results}
The EVP model defines an agent's motivation as a weighted combination of
three normalized components:

\begin{equation}
m_i = \frac{w_E E_i + w_V V_i + w_P P_i}{w_E + w_V + w_P},
\label{eq:motivation}
\end{equation}
where \( E_i \) represents expectancy, \( V_i \) the value, and \( P_i \) the payoff, while \(w_E\), \(w_V\), and \(w_P\) denote their relative weights. Expectancy quantifies how likely an action is to succeed, value represents the agent's intrinsic interest in succeeding, and payoff captures how advantageous the agent's current relative position is within the crowd. These three quantities depend on the setting and on the position of the pedestrian in that setting. In particular, they depend on the distance to the entrance, the relative rank within the crowd, and the subjective value attached to the outcome.

\paragraph{Expectancy.}  
The expectancy \( E(d) \) depends on the agent's distance \( d \) to the exit. It is modeled as a bell-shaped function:

\begin{equation}
E(d) =
\begin{cases}
e \cdot \exp\left(\dfrac{1}{\left(\frac{d}{w}\right)^2 - 1}\right), & d < w, \\
0, & d \ge w,
\end{cases}
\label{eq:expectancy}
\end{equation}
where \( w \) is the influence radius. In the current implementation, \(E(d)\)
is normalized to the interval \([0,1]\): it reaches its maximum value \(1\)
at the goal and drops to \(0\) at and beyond \(w\). The parameter \(h\) appears
in the intermediate computation in the code, but it cancels out in the final
normalized expectancy value.
...
In the implementation, these absolute values are then normalized to the
interval \([0,1]\) before entering the motivation model:
\begin{equation}
V_i = \frac{v_i - v_{\min}}{v_{\max}-v_{\min}},
\end{equation}
with clipping to \([0,1]\). Thus, \(V_i\) denotes the normalized value term
used in the EVP model. The resulting motivation is therefore a weighted
average of the normalized expectancy, value, and payoff terms.
...
\begin{table}[htbp]
  ...
  \begin{tabular}{ll}
    ...
    High value range & [1.5, 1.8] \\
    Low value range & [0.2, 0.4] \\
